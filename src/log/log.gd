...
func temp_scope(_temp_scope: String) -> Log:
	var t = Log.new()
...
func warn(message: String, spacer: bool = false) -> void:
	if !is_log_enabled("WARN"): return
	push_warning(format_log("", message))
	if spacer: Log.print_spacer()

## Prints an error log
func error(message: String, spacer: bool = false) -> void:
	if !is_log_enabled("ERROR"): return
	push_error(format_log("", message))
	if spacer: Log.print_spacer()
